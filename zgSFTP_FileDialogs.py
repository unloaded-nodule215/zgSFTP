...
class host_key_dialog:
    ...
    def __init__(self, master, Title, icon, host, port, key_type, fingerprint):
        ...
        center_window(master, self.host_key_dialog_window)
...
class known_hosts_dialog:
    ...
    def __init__(self, master, Title, icon):
        self.known_hosts_window = Toplevel(master)
        self.known_hosts_window.resizable(True, False)
        self.known_hosts_window.title(Title)
        self.known_hosts_window.minsize(width = 500, height = 300)

        self.icon_frame = ttk.Frame(self.known_hosts_window)
        self.icon_frame.pack(side = 'top', fill = X, padx = 5, pady = 5)

        ttk.Label(self.icon_frame, image = icon).pack(side = 'left', padx = 3)
        ttk.Label(self.icon_frame, text = 'Known SSH Hosts').pack(side = 'left', padx = 5)

        self.list_frame = ttk.Frame(self.known_hosts_window)
        self.list_frame.pack(fill = BOTH, expand = True, padx = 5, pady = 5)

        self.canvas_frame = ttk.Frame(self.list_frame, relief = 'groove', border = 1)
        self.canvas_frame.pack(fill = BOTH, expand = True)

        self.vbar = ttk.Scrollbar(self.canvas_frame, orient=VERTICAL)
        self.vbar.pack(side=RIGHT, fill=Y)

        self.host_listbox = Listbox(self.canvas_frame, yscrollcommand=self.vbar.set)
        self.host_listbox.pack(fill=BOTH, expand=True)
        self.vbar.config(command=self.host_listbox.yview)

        self.button_frame = ttk.Frame(self.known_hosts_window)
        ...
        self.remove_button.pack(side = 'right', padx = 3)

        self.clear_all_button = ttk.Button(self.button_frame, text = 'Clear All', command = self.clear_all)
        self.clear_all_button.pack(side = 'right', padx = 3)

        self.close_button = ttk.Button(self.button_frame, text = 'Close', command = self.known_hosts_window.destroy)
        self.close_button.pack(side = 'right', padx = 3)

        center_window(master, self.known_hosts_window)
        self.known_hosts_window.transient(master)
        self.known_hosts_window.focus_force()

        self.known_hosts = host_keys.list_known_hosts()
        self.host_to_index = {}

        for i, (host, port, key_type, fingerprint) in enumerate(self.known_hosts):
            display_text = f"{host}:{port} ({key_type})"
            self.host_listbox.insert(END, display_text)
            self.host_to_index[display_text] = i

        max_attempts = 10
        for _ in range(max_attempts):
            try:
                self.known_hosts_window.grab_set()
                break
            except Exception:
                import time
                time.sleep(0.1)
    ...
    def remove_selected(self):
        try:
            ...
            for index in reversed(selection):
                display_text = self.host_listbox.get(index)
                parts = display_text.split(':')
                if len(parts) >= 2:
                    host = parts[0]
                    port = parts[1].split(' ')[0]
                    host_keys.remove_known_host(host, port)
                    self.host_listbox.delete(index)

            self.known_hosts = host_keys.list_known_hosts()
            self.host_to_index.clear()
            for i, (host, port, key_type, fingerprint) in enumerate(self.known_hosts):
                display_text = f"{host}:{port} ({key_type})"
                self.host_listbox.insert(END, display_text)
                self.host_to_index[display_text] = i
    ...
    def destroy(self):
        self.known_hosts_window.destroy()